...
## Как это работает
Телефон (Android) — сервер, отдаёт данные. Компьютеры (macOS/Windows) — клиенты: подключаются и показывают. Сопряжение по QR (клиент показывает, телефон сканирует). Транспорт: Wi-Fi (mDNS) → Bluetooth LE → облачный relay; приоритет у локальной связи. Relay — опциональный self-host для облачного транспорта, когда нет общей сети/Bluetooth.
